...
export class AuthBloc {
  ...
  constructor(
    private readonly userService: UserBloc,
    private readonly roleService: RoleService,
    private readonly tokenProvider: TokenProvider,
    private readonly log: AppLogger,
  ) {
    this.context = AuthBloc.name;
  }

  async login(email: string, password: string): Promise<any> {
    this.log.info(this.context, `Login by email #${email}`);

    const user = await this.userService.getByEmail(email);
    if (!user) {
      throw new UnauthorizedException(this.context, 'Invalid email or password!');
    }
  ...
  refreshToken(refreshToken: string): RefreshTokenResDto {
    this.log.info(this.context, 'Refresh token');
